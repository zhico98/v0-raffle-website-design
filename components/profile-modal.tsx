...
import { getTransactions, calculateUserStats, type Transaction } from "@/lib/transaction-storage"
...
export function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
  ...
  useEffect(() => {
    if (isOpen && account) {
      ...
      const userStats = calculateUserStats(userTransactions)
      setStats(userStats)

      if (userTransactions.length > 0) {
        const firstTx = userTransactions.sort((a, b) => a.timestamp - b.timestamp)[0]
        setMemberSince(new Date(firstTx.timestamp).toLocaleDateString())
      } else {
        setMemberSince(new Date().toLocaleDateString())
      }

      console.log("[v0] Loaded user stats:", userStats)
      console.log("[v0] Loaded transactions:", userTransactions.length)
    }
  }, [isOpen, account])
